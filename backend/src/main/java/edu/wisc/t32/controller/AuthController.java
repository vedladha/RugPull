 ...
 */
// TODO: Implement JWT token generation and validation in the AuthService and JwtUtil classes,
// and update the login and profile endpoints accordingly to use JWT for authentication and
// authorization.
@RestController
@RequestMapping("/auth")
public class AuthController {
   ...
   *
   * @param authService           the service responsible for handling authentication logic such as
   *                              registration and login
   * @param userRepository        the repository used to interact with the User data in the
   *                              database, such as checking for existing emails during registration
   *                              and retrieving user information during login
   * @param userProfileRepository the repository used to interact with the UserProfile data in the
   *                              database, such as retrieving user profile information during login
   */
  ...
  /**
   * Register endpoint creates a new user account with the provided display name, email,
   * and password.
   *
   * <p>The password is currently stored in plain text for initial setup and testing, but will be
   * hashed and salted in a future implementation.
   *
  ...
  /**
   * Login endpoint authenticates the user and returns a JWT token in an HTTP-only cookie.
   * The token will be used for subsequent requests to protected endpoints.
   *
   ...
   * @return the user's email and display name if the token is valid, or an error if the
   *         token is invalid or expired
   */
  @GetMapping("/profile")
  public ResponseEntity<?> getProfile(@CookieValue(name = "jwt") String token) {
    return ResponseEntity.ok(Map.of("message",
        "This endpoint will return the user's profile information based on the JWT token in a"
